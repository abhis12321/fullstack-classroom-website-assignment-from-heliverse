...
export default function UserRegister({ setOption }) {
    ...
    return (
        <div className="fixed top-0 left-0 z-20 h-[100vh] w-full flex items-center justify-center bg-gray-700/90" >
            <form className='text-gray-950 from-blue-50 to-blue-50 bg-gradient-to-b shadow-[0_0_4px_black] w-[98%] max-w-[600px] rounded-2xl px-1 sm:px-2 py-2 xs:py-[10px] gap-1 flex flex-col items-center justify-center' onSubmit={handleUserRegistration}>
                <div className={`w-full flex items-center justify-around pb-2 drop-shadow-[0_0_2px_black]`}>
                    <h1 className="flex rounded-lg text-3xl lg:text-4xl font-bold sm:font-extrabold text-center w-fit text-red-900 drop-shadow-[0_0_1px_white]">New Resistration</h1>
                </div>
                <div className="flex flex-row sm:flex-row gap-2 items-center justify-evenly w-[98%] max-w-[600px]">
                    ...
                    <select name="status" value={userRole} onChange={(e) => setRole(e.target.value)} className='w-[120px] outline-none invalid:text-pink-600 font-semibold bg-blue-800/15 hover:bg-indigo-800/25 shadow-[0_0_10px_gray_inset] focus:shadow-[0_0_10px_brown_inset] py-[6px] px-3 rounded-md text-center ring-cyan-600 ring-2' >
